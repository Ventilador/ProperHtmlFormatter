// NOTE: we use .js because .ts doesn't seem to play well with dynamic async
// imports through webpack yet (see "const WASM = await import ..." below).

import * as vscode from "vscode";

export async function activate(context: vscode.ExtensionContext) {
  vscode.window.showInformationMessage('before');
  const formatter = await import("rust-html-formatter");
  vscode.window.showInformationMessage('after');
  vscode.window.showInformationMessage(formatter.greet('pedro'));

  context.subscriptions.push(vscode.languages.registerDocumentFormattingEditProvider('html', {
    provideDocumentFormattingEdits: (document) => {
      vscode.window.showInformationMessage('after');
      console.log('asd');
      vscode.window.showInformationMessage('leee');
      let start = new vscode.Position(0, 0);
      let end = new vscode.Position(document.lineCount - 1, document.lineAt(document.lineCount - 1).text.length);
      const range = new vscode.Range(start, end);
      return [new vscode.TextEdit(range, document.getText(range))];
    }
  }));
